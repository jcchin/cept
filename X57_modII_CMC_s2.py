...
# Change flow resistances solve for two different flow rates
# Scenario 1: Motor + 3/8" annulus + wing duct
# Scenario 1b: Motor + 3/8" annulus
# Scenario 2: Wing duct exclusively to controller
...
cowl_gap = 9.5E-3 # 3/8" gap
d_inner = 0.352 #m motor inner diam
d_outer = 0.371 # motor outer diameter
cowl_area = np.pi*((cowl_gap+d_outer)**2.)/4 - np.pi*(d_outer**2.)/4.
motor_area = 0.009 # m^3
wing_hole_area = (np.pi*(50E-3)**2.)/4.
...
def dxdt(temp, t):
    ...
    wingMdot = mdot * (wing_hole_area/(cowl_area+motor_area+wing_hole_area))  # roughly 11.8%
    annulusMdot = mdot * (cowl_area/(cowl_area+motor_area+wing_hole_area))  # roughly 33.8%
    T_total = T_0#(T_motor_ex*mdot + wingMdot*T_0 + annulusMdot*T_0)/(mdot+wingMdot+annulusMdot)
    #print(cowl_area/(cowl_area+motor_area+wing_hole_area))
    #print(T_motor_ex, T_total)
    ...
    Q_conv_CMC = U_conv_cmc*(temp[0]-T_total)
    #################
